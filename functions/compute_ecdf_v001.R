...
###############################################################################
# This function returns the empirical cumulative distribution function. It
...
compute_ecdf <- 
   ...
   f <- rep( 0, n_unq )                 # Set up vector to hold frequency of
    ...
    for( i_unq in 1:n_unq )             # Loop through each unique value ....
    {
       for( j_all in 1:n_all ){         # Loop through original values ....
   ...
   cf <- cumsum( f )                    # Compute cumulative frequency ...
   
   ecdf <- cf / cf[ n_unq]              # Compute the relative cumulative
                                        # frequency ...
